...
const Tasks = () => {
  ...
  const handleTaskCardClick = async (task) => {
    ...
    try {
      ...
      setModalProjectData(data);
    } catch (err) {
      console.error('Failed to fetch project for task modal:', err);
    } finally {
  ...
  const handleUpdateTask = async (taskId, taskData) => {
    try {
      ...
      setSelectedTask((prev) => (prev?._id === taskId ? { ...prev, ...updatedTask } : prev));
    } catch (err) {
  ...
  // Resolve the list name for the selected task from modal project data
  const activeTaskListName =
    selectedTask && modalProjectData
      ? modalProjectData.lists?.find(
          (l) => l._id === (selectedTask.list?._id || selectedTask.list)
        )?.name || 'Backlog'
      : '';

  if (loading)
    return <div className="p-10 max-w-7xl mx-auto w-full flex-1">Loading tasks...</div>;
        ...
        <div className="max-w-7xl mx-auto space-y-stack-lg">
                ...
                {tasks.map((task) => {
                  ...
                  return (
                    <div
                      ...
                      className="bg-surface-container-lowest p-5 rounded-xl border-l-4 border-secondary shadow-sm hover:-translate-y-1 hover:shadow-md transition-all group cursor-pointer"
                    >
                      <div className="flex justify-between items-start mb-3">
                        <span className={`${pStyle.class} px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider`}>
                          {pStyle.label}
                        </span>
                        <i className="fa-solid fa-arrow-up-right-from-square text-on-surface-variant text-sm opacity-0 group-hover:opacity-100 transition-opacity"></i>
                      </div>
                        ...
                        <div className="flex -space-x-2">
                          {task.assignee && (
                            <img
                              ...
                              className="w-7 h-7 rounded-full border-2 border-surface-container-lowest bg-surface-container"
                              src={task.assignee.avatarUrl || `https://openui.fly.dev/openui/80x80.svg?text=${task.assignee.name.charAt(0)}`}
                            />
                          )}
                        </div>
          ...
          <section className="grid grid-cols-1 md:grid-cols-12 gap-6 pb-12">
            ...
            <div className="col-span-1 md:col-span-4 bg-surface-container-lowest p-6 rounded-2xl shadow-sm border border-outline-variant/10">
              ...
              <div className="text-[40px] font-bold text-on-surface leading-none">
                {completionVelocity} <span className="text-secondary text-label-md">tasks/day</span>
              </div>
            ...
            <div className="col-span-1 md:col-span-8 bg-surface-container-lowest p-6 rounded-2xl shadow-sm border border-outline-variant/10 flex items-center justify-between">
              ...
              <div className="flex flex-col items-center px-4">
                ...
                <span className="text-[10px] text-on-surface-variant mt-2 text-center uppercase tracking-tighter">
                  Live<br />Optimization
                </span>
              </div>
            </div>
          </section>
        ...
        <div className="fixed inset-0 z-50 md:left-64 bg-background flex flex-col">
          {modalLoading || !modalProjectData ? (
            ...
            <TaskModal
              ...
              projectMembers={modalProjectData.project?.members}
